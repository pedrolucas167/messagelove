...
const validationSchemas = {
  register: {
    email: { in: ['body'], isEmail: { errorMessage: 'Email inválido' }, customSanitizer: { options: normalizeEmail }, custom: { options: async (value) => { const user = await User.findOne({ where: { email: value } }); if (user) throw new Error('Email já está em uso'); return true; } } },
    password: { in: ['body'], isLength: { options: { min: 8 }, errorMessage: 'Senha deve ter pelo menos 8 caracteres' }, matches: { options: /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}$/, errorMessage: 'Senha deve conter letras maiúsculas, minúsculas e números' } },
    name: { in: ['body'], notEmpty: { errorMessage: 'Nome é obrigatório' }, isLength: { options: { max: 120 }, errorMessage: 'Nome deve ter no máximo 120 caracteres' }, trim: true }
  },
  login: {
  ...
  reset: {
    ...
    newPassword: { in: ['body'], isLength: { options: { min: 8 }, errorMessage: 'Senha deve ter pelo menos 8 caracteres' }, matches: { options: /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}$/, errorMessage: 'Senha deve conter letras maiúsculas, minúsculas e números' } }
  }
};
...
const authHeaderVerify = (req, res, next) => {
  try {
...
router.get('/verify', authHeaderVerify, (req, res) => {
...
router.post('/login', authLimiter, checkSchema(validationSchemas.login), validateRequest, async (req, res, next) => {
  try {
    const email = normalizeEmail(req.body.email);
    const password = String(req.body.password || '');
    const user = await User.unscoped().findOne({ where: { email }, attributes: ['id', 'name', 'email', 'password', 'created_at', 'createdAt'] });
